...
// 在一个新线程中运行发布者，根据给定的样本数和睡眠时间发送消息
void CustomListenerPublisher::runThread(uint32_t samples , uint32_t sleep)
{
    if (samples == 0)
    {
        while (!stop_)
...
// 启动发布者，根据给定的样本数和睡眠时间
void CustomListenerPublisher::run(uint32_t samples, uint32_t sleep)
{
    stop_ = false;
...
// 发布消息，根据是否等待监听器来决定
bool CustomListenerPublisher::publish(bool waitForListener)
{
